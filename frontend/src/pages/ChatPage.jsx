...
import { Send, MessageSquare, Plus, Search, X, User as UserIcon } from 'lucide-react';
import Toast from '../components/Toast';

// Modal component for finding and starting a new DM
      ...
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        ...
        <ul className="mt-4 max-h-60 overflow-y-auto">
          ...
          {!isLoading && results.map(user => (
            <li key={user.id} onClick={() => onSelectUser(user.id)} className="p-3 hover:bg-gray-100 rounded-md cursor-pointer font-medium">
              {user.name}
            </li>
          ))}
          {!isLoading && query && results.length === 0 && <li className="p-2 text-gray-500">No users found.</li>}
        </ul>
      </div>
    </div>
  );
...
export default function ChatPage() {
  ...
  const { messages, status: chatStatus, connectionStatus, error } = useSelector(selectChat);
  ...
  // Manage the WebSocket connection and chat history for the active chat
  useEffect(() => {
    if (activeGroupId) {
      dispatch(fetchChatHistory(activeGroupId));
      dispatch(startChatSession(activeGroupId));
    }
    // Cleanup function: disconnects socket when component unmounts or active chat changes
    return () => {
  ...
  const handleStartDm = async (targetUserId) => {
    ...
    if (startDirectMessage.fulfilled.match(resultAction)) {
      ...
      // Optimistically add the new conversation to our local list for an instant update
  ...
  const activeGroup = myConversations.find(group => group.id === activeGroupId);
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)]">
      {error && <Toast message={error} onClose={() => { /* You might need a clearChatError action here */ }} />}
      <NewMessageModal 
      ...
      <aside className="w-1/4 bg-white border-r overflow-y-auto">
        <header className="p-4 border-b flex justify-between items-center">
          ...
          <button onClick={() => setIsNewMessageModalOpen(true)} className="p-2 rounded-full hover:bg-gray-200">
            <Plus />
          </button>
        </header>
        <ul>
          {myConversations.map(group => {
            ...
            let subtext = group.is_direct_message ? "Direct Message" : group.hobby;
              ...
              <li key={group.id}>
                ...
                >
                  <p className="flex items-center gap-2">
                    {group.is_direct_message && <UserIcon size={16} className="text-gray-500" />}
                    {displayName}
                  </p>
